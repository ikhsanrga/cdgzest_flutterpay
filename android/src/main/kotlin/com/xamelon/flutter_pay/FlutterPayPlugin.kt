    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?): Boolean {
        if (requestCode == LOAD_PAYMENT_DATA_REQUEST_CODE) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                this.lastResult?.error("userCancelledError", "User cancelled the payment", "User cancelled the payment")
            } else if (resultCode == AutoResolveHelper.RESULT_ERROR) {
                val status = AutoResolveHelper.getStatusFromIntent(data)
                print("AutoResolveHelper.RESULT_ERROR")
                print("Status: ${status?.toString()}")
                this.lastResult?.error("paymentError", "Google Pay returned payment error with status: ${status?.toString()}",
                        "Status Error Message: ${status?.statusMessage}")
            }
